...
function solution(arr) {
  ...
  // 각 행의 합
  arr.forEach((arrValue) => resultSet.add(arrValue.reduce((result, curr) => ((result += curr), result), 0)));

  // 각 열의 합
  for (let colIdx = 0; colIdx < arr[0].length; colIdx++) {
    let sum = 0;
    for (let rowIdx = 0; rowIdx < arr.length; rowIdx++) sum += arr[rowIdx][colIdx];
    resultSet.add(sum);
  }
